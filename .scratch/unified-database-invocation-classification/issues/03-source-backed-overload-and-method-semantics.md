...
- [ ] `Receiver Implementation Binding` 會沿著 receiver expression、construction 與 relevant assignments 解析 concrete implementation；可取得時保留 exact assembly identity 與 assembly revision，receiver type name 單獨不能選擇 contract 或 overload。
- [ ] 每個可分類的 method 都保留 bound implementation、receiver type、method name、arity 與可取得的 parameter types，並以這些 facts 選擇 overload。
...
- [ ] Source-backed SQLObject fixture 能驗證 text-mode reader/scalar/edit、fixed stored-procedure reader/non-query、call-site semantics 與 concrete receiver binding。
