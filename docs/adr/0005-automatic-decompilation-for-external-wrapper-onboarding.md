...
**Amended by ADR 0006**: an early implementation of this decompile-based onboarding flow added a registry-wide, receiver-type-name-based skip that this decision never specified, and that defeated the "no human review step" guarantee above whenever another system's contract happened to share a receiver-type name. That skip is removed; reuse-versus-create is decided solely by Contract Fingerprint. See [0006-fingerprint-only-reuse-for-decompile-based-contract-onboarding.md](./0006-fingerprint-only-reuse-for-decompile-based-contract-onboarding.md).
