...
| 从未執行過 `refresh_sql_cli`，問問題仍有 SP 定義 | 快取不存在時，`sp_fetcher.py` 會自動 fallback 即時連線查詢（與舊行為一致）；`view_fetcher.py` 則只讀本機快取，尚未快取時不會有 View 定義（不影響一般資料表分析）。 || 不確定哪些系統忘記 clone/掃描/更新 SQL 快取 | 執行 `python tools\check_data_status.py`（見 Step 5）一次盤點所有系統，會直接列出需要補跑的 `refresh_cli`/`refresh_sql_cli` 指令。 |
---
