...
> 原生依賴、`SQL Execution Graph` 與寫入 cache 的進度；不需要另外盯著啟動
> `uvicorn` 的 terminal。直接呼叫同步 `POST /refresh_sql` 的整合程式，也可以自行
> 提供 `job_id` 後輪詢同一個 status endpoint。
>
...
| **SQL 全庫撈取進度顯示** | `sql_analyzer.py` 的 `dump_all_sql_objects()` 與 `sql_execution_graph.py` 回報逐階段計數；`service/api.py` 暫存 job 狀態，`refresh_sql_cli.py` 以 `ThreadPoolExecutor` 背景等待同步 POST 並輪詢 `/refresh_sql/status/{job_id}`，讓進度條在 CLI 同一個 terminal 顯示 |
