...
│   ├── api.py                       # /health /analyze /refresh /refresh_sql
...
> / `refresh_sql_cli.py`（更新 SQL 快取）、`sp_lookup.py`（以 SP 名稱反查程式，供 agent 模式使用）。
...
[rag_client.analyze()] ──── HTTP POST /analyze ────▶ [service/api.py]
                                                  ...
                                                  [analyze_service.analyze()]
                                                     ...
                                                     ├─ fk_resolver         FK 連動表（需 DB 即時連線）
                                                     ├─ sp_fetcher          SP 完整定義（優先讀本機 SQL 快取）
                                                     ├─ view_fetcher        SQL View 完整定義（同樣優先讀快取）
...
> 💡 **mode 3（agent）支援以 SP 名稱反查程式**：当問題主要是提到一支具體 SP（如「usp_POPublish 發生...」）而沒有畫面/功能關鍵字時，agent 會自動改用 `search_specs_by_sp` 工具（對應 Impact 的 `POST /find_by_sp`）逐系統反查哪些程式呼叫過該 SP，找到後再自動取得對應規格書並進行静態分析。此反查只比對**已 clone 且已掃描過**的系統（`cache_only=True`），尚未 clone/掃描過的系統會直接跳過、**不會**觸發 clone；若某支 SP 在尚未掃描過的系統裡，需先執行 Step 3/5 確保該系統已 clone+掃描。
---
...
| **以 SP 名稱反查程式（`/find_by_sp`）** | 訮問只提到 SP、沒畫面/功能關鍵字時，舊流程（規格書/程式名萍取）找不到入口；新增 `find_by_sp(req)` 比對已快取的 `sp_relations`，不分大小寫比對 SP 名稱；`cache_only=True`（預設）時若 repo 尚未 clone/掃描過即直接 `skipped=True`、不觸發 clone，因為此端點常被逐 catalog 系統投機式呼叫，必須避免每次未命中都觸發昂貴 clone；spec-rag 側只在 **agentic 模式（mode 3）**自動呼叫，不影響現有確定性流程 |
