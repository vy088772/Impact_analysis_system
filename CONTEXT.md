...
**Command Source**:
The construct inside one wrapper method that supplies the method's command text and terminal sink. A method has a Command Source when a resolution rule recognizes its construct — today an explicit command object construction or a command object obtained through a factory call such as a connection's `CreateCommand()` and bound to a locally declared variable of a command type, a data adapter construction taking a command text argument and a connection argument, or raw SQL executed directly through a database context's own `Database` facade (`ExecuteSqlRaw`, `ExecuteSqlInterpolated`, and their asynchronous forms). A command object construction is recognized by the contract it implements first: a type is a command object when it implements `IDbCommand`, whichever provider or team wrote it, so constructing one named type is never the only recognized construct. When the analyzer cannot answer that contract question — an assembly whose framework has no reference assemblies, or a symbol a partially-bound synthetic source could not resolve — the rule falls back to a name comparison against the abstract ADO.NET command type and a multi-provider list of provider-specific types. The two mechanisms answer one question and produce one outcome; nothing in the reported result records which of the two resolved a given Command Source. The `Database`-facade shape constructs no command object at all and binds no command variable: its command text is the argument the caller supplies to the execution call, traced back through any local-variable indirection to the method's own parameter, and the execution call itself is the terminal sink; its Connection Behavior Boundary is always `context_connection`, the same last-resort receiver reading a command factory's own connection already uses. The deferred `FromSqlRaw`/`FromSqlInterpolated` forms are not this shape — they return a queryable, execute later, and hang off a `DbSet` rather than the `Database` facade, so they stay unrecognized. A method that touches a database type but yields no Command Source is a visible gap, not a silent drop.
_Avoid_: SqlCommand construction, command builder

**Connection Behavior Boundary**:
Where one wrapper method's connection comes from, recorded per operation in a Contract's behaviour signature. `constructor_connection` means the caller handed the connection to the wrapper, so the wrapper opens what it was given. `wrapper_connection` means the wrapper holds its own connection, the shape a Field-Held Connection produces — including one reached through a command factory call, e.g. `connection.CreateCommand()`, whose receiver the resolver reads only once the constructor-argument rule and a `Connection` property assignment both resolve nothing. `context_connection` means that same factory receiver is a database context's own `Database` facade instead, e.g. `Database.GetDbConnection().CreateCommand()`; the actual database then follows from the call site's declared receiver type through Context Connection Registration, not from a connection the wrapper holds itself — `SQLDbContext` serves five Systems and many derived context types, so `wrapper_connection` would record a connection that does not exist. A method whose connection the resolver could not read reports an empty boundary, which fails the behaviour surface completeness check with `connection_behavior_boundary_missing` rather than passing as a guess. The value is recorded, never branched on: it is part of the Contract Fingerprint, so two wrappers that differ only here are two Contracts. See [ADR-0025](docs/adr/0025-command-factory-connection-gains-a-third-boundary-value.md).
