...
Where one wrapper method's connection comes from, recorded per operation in a Contract's behaviour signature. `constructor_connection` means the caller handed the connection to the wrapper, so the wrapper opens what it was given. `wrapper_connection` means the wrapper holds its own connection, the shape a Field-Held Connection produces. A method whose connection the resolver could not read reports an empty boundary, which fails the behaviour surface completeness check with `connection_behavior_boundary_missing` rather than passing as a guess. The value is recorded, never branched on: it is part of the Contract Fingerprint, so two wrappers that differ only here are two Contracts.
